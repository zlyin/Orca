...
class CyclicLR(Callback):
    ...
    def __init__(self, base_lr=0.001, max_lr=0.006, step_size=2000., \
        ...
        # return next class of CyclicLR in the MRO/method resolution order
        ...
        # check if custom scale_fn is defined; 
        # if not, feed in a lambda function
        if scale_fn == None:
    ...
    def clr(self):
        # indicate position within a cycle
        cycle = np.floor(1 + self.clr_iterations / (2 * self.step_size))
        x = np.abs(self.clr_iterations / self.step_size - 2 * cycle + 1)

        if self.scale_mode == 'cycle':
            delta = (self.max_lr - self.base_lr) * np.maximum(0, (1 - x)) * \
                    self.scale_fn(cycle)
        else:
            delta = (self.max_lr - self.base_lr) * np.maximum(0, (1 - x)) * \
    ...
    def on_train_begin(self, logs={}):
        ...
        # check if just start training of each epoch
